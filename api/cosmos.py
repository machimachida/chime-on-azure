...
MEETING_CONTAINER_ID = 'meetings'
CONTAINER_ID = 'testcon'
USER_AGENT = 'chime-on-azure'

# データベースからmeeting情報が存在するドキュメントを取り出す。
def getMeeting(meeting_title: str) -> dict:
    ...
    try:
        ...
        container = read_container(database, MEETING_CONTAINER_ID)
        items = container.query_item(
            query="SELECT * FROM r WHERE r.title=@title",
            parameters=[
                {"name": "@title", "value": meeting_title}
            ]
        )
        return items[0] # TODO: とりあえずtitleの重複は無視してる
...
def putMeeting(title: str, meeting_info: dict):
